...
// Memanggil kolom yang ada pada tabel pengajuan pembuangan aset
$query_mysql = mysqli_query($mysqli, "SELECT * FROM pengajuan_pembuangan");
...
<head>
    ...
    <title>Pengajuan Pembuangan Aset</title>
...
<body>
        ...
        <div class="container"> 
            ...
            <div class="header">
                <div class="menu-bar">
                    ...
                    <h1>Pengajuan Pembuangan Aset</h1>
                </div>
                <ul>
            ...
            <div class="box">
                <h2>Persetujuan Pembuangan Aset</h2>
                <table>
                    <thead>
                        <tr>
                            <th>ID</th>
                    ...
                    <?php while ($data = mysqli_fetch_assoc($query_mysql)) { ?>
                        <tr>
                            <td><?php echo $data['idPengajuanPembuangan']; ?></td>
                            <td><?php echo $data['idAset']; ?></td>
                            <td><?php echo $data['namaAset']; ?></td>
                            <td><?php echo $data['kategoriAset']; ?></td>
                            <td><?php echo $data['jumlah']; ?></td>
                            <td><?php echo $data['departemen']; ?></td>
                            <td><?php echo $data['tanggalPembuangan']; ?></td>
                            <td><?php echo $data['alasanPembuangan']; ?></td>
                            <td>
                                <div class="status-text <?php echo ($data['status'] == 'APPROVED') ? 'status-approved' : (($data['status'] == 'REJECTED') ? 'status-rejected' : ''); ?>">
                                    <?php echo $data['status']; ?>
                                </div>
                                <?php if (empty($data['status'])) { ?>
                                    <button class="approve" data-id="<?php echo $data['idPengajuanPembuangan']; ?>">Approve</button>
                                    <button class="reject" data-id="<?php echo $data['idPengajuanPembuangan']; ?>">Reject</button>
                                <?php } ?>
                            </td>
                        </tr>
                        <?php } ?>
    ...
    <script>
        ...
        function updateStatus(button, status) {
            ...
            xhr.open('POST', 'php/statusPembuangan.php', true);
